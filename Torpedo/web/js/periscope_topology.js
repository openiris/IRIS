...
iris.topologyCollection = new Topology();
...
//	console.log('loading topology');
	...
	iris.topologyCollection.once('change', function() {
		...
		var nodes = iris.topologyCollection.nodes;
		var links = iris.topologyCollection.links;

		iris.topologyCollection.nodes.forEach(function (d) {
...
//		console.log(iris.topologyCollection.nodes);
		...
		iris.hostCollection.models.forEach(function (d) {
			...
			d.fixed = false;
			d.weight = 0;
			if (d.attributes['ipv4'].length > 0) {
                d.name = d.attributes['ipv4'][0] + "(" + d.id + ")";
            } else {
                d.name = d.id;
            }
			d.group = 2;		/* host */
		...
		nodes = nodes.concat(iris.hostCollection.models);
...
//		console.log(nodes);
		...
		iris.topologyCollection.links.forEach(function (d) {
		...
		for (var i = 0; i < iris.hostCollection.models.length; i++) {
            var host = iris.hostCollection.models[i];
