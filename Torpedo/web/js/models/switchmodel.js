	...
	loadFlows:function () {
		...
		$.ajax({
			...
			success:function (data) {
				...
				console.log("number of flows are = " + flows.length);
				if ( flows.length <= 0 ) {
					self.flows.reset();
					return;
				}
...
window.SwitchCollection = Backbone.Collection.extend({
	...
	initialize:function () {
		...
		$.ajax({
			...
			success:function (data) {
				...
				if ( data.length <= 0 ) {
					console.log('reset switch collection');
					self.reset();
					self.trigger('remove');
				}
				else {
	...
	fetch:function () {
		this.initialize();
		_.each(this.models, function(model){ model.fetch();});
	}
